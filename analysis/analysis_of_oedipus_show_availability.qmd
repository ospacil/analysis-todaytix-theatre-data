...
2. **Some performances later in the show run do not have the "full" number of data points.** We shall have a look in detail, but it seems that we might be missing some of the daily data points for these performances.
...
Looking at data points for a single performance, we can see that the missing dates are squeezed between dates when seat availability was close to 0:
